...
public class AssociadoController {
    ...
    @Operation(summary = "Busca a listagem de associados com paginação")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Listagem dos associados")
    })
    public ResponseEntity<Page<AssociadoResponseDTO>> buscarAssociadoPaginado(
            @RequestParam(name = "pagina") Integer pagina,
            @RequestParam(name = "tamanho") Integer tamanho
    ) {
        return ResponseEntity.ok(associadoService.buscarAssociadoPaginado(pagina, tamanho));
    }
}
